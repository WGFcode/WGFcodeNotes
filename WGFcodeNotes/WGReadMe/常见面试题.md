...
* 使用alloc方法时，它只是为NSMutableArray对象分配了内存空间,还没有被正确地初始化，它处于一种未完全可用的状态
* 如果直接使用[NSMutableArray alloc]生成的数组去添加数据会导致程序crash
* init方法是用于初始化刚刚通过alloc分配内存的对象的;init方法会完成一系列必要的设置(设置初始容量/初始化用于存储元素的内部数据结构等)

#### 延伸: alloc一个对象 和 alloc init区别
* alloc是 ObjectiveC 中用于分配内存的方法。它的主要功能是在堆（heap）上为对象分配足够的内存空间，使得对象能够在内存中存在
* 仅使用alloc分配内存后，对象的内存空间只是被预留出来了，但对象还没有被正确初始化；对象还不能正常使用
* init方法用于初始化刚刚通过alloc分配内存的对象。它的主要目的是将对象的实例变量设置为合理的初始值，使得对象能够正常使用

### 28.项目中数据库如何进行升级
#### 背景: 之前数据库中已经定义好的字段内容，因为业务需求需要拓展数据库字段，如何保证线上用户版本升级app后数据库数据能够保证不出错
1.首先需要确定数据库更新策略: 

        如果是简单的操作(比如添加字段)等，可以使用 SQLite 的ALTER TABLE语句直接在现有表结构上修改
        若涉及到复杂的数据库结构变化(比如改变数据类型、合并或拆分表)，可能需要创建新表并迁移数据
        
2.考虑备份方案：

        更新数据库之前，最好能够备份整个数据库文件或者关键的数据表，可以使用NSFileManager在 iOS 设备的本地存储中备份数据库
        将数据库文件复制到应用的文档目录下的一个备份文件夹中（在更新出现问题时，可以有机会恢复数据，避免用户数据丢失）
        
3. 严格管理数据库的版本号：

        通过框架提供的版本管理机制来跟踪数据库的版本；每次更新数据库结构时，确保版本号正确递增
        应用启动时检查数据库版本号，并根据不同的版本号执行相应的更新操作

4.SQL 语句的兼容性

        编写 SQL 语句时要考虑兼容性，应用可能会在不同版本的 SQLite 上运行，要确保使用的 SQL 功能在这些版本中都能正常工作

5.数据迁移测试
6. 版本回滚计划

        通过服务器端控制应用是否执行数据库更新，一旦发现问题，阻止新用户进行更新，并引导已更新的用户回滚数据库版本

### 29.iOS中的编码方式
#### 29.1 UTF-8编码:
* 支持全球字符集: 国际化的应用程序中，当需要显示多种语言的文本内容时，UTF - 8编码能够确保这些字符被正确地存储和显示
* 与 ASCII 兼容
* UTF-8是可变长度编码。它使用1-4个字节来表示不同的 Unicode 字符;能够根据实际字符的复杂程度灵活分配空间
* 网络通信中,UTF - 8 编码都能够确保数据的正确传输和不同系统之间的兼容性
* UTF-8编码优势

        1.支持多种语言：与ASCII编码相比ASCII编码只能表示 128 个字符，而UTF-8是Unicode的一种编码形式，可以表示全球几乎所有语言的字符
        2.UTF-8是可变长度编码，根据字符不同，使用1-4个字节表示，空间利用率更高
        3.存储和传输效率高
        4.兼容性和通用性好
        ...
        //2.将Data二进制 -> 字符串
        let newStr = String.init(data: data, encoding: .utf8) ?? ""
        
        NSLog("-------\(data)----\(data.count)----\(newStr)")
        -------11 bytes----11----Hello world

        swift中用Data这个结构体来表示二进制数据
        
#### 29.2 UTF-16编码
#### UTF-16 主要用于表示 Unicode 字符；UTF-16通常使用2个字节来表示，罕见的字符需要用4个字节来表示
在某些需要对文本进行复杂内部处理的场景下，UTF - 16 的固定长度特性（对于多数字符）可以简化字符处理的逻辑

#### 29.3 ASCII 编码
#### ASCII 编码是最基本的字符编码方式，主要用于表示英文字母、数字、标点符号和一些控制字符；它只使用 7 位（或 8 位扩展
 ASCII 使用 8 位）来表示字符，总共可以表示 128 个
 * ASCII编码规则简单，编码后的字节长度固定，每个字符占用 1 个字节。
 * 局限性：只能表示有限的字符集，无法表示其他语言的字符，如中文、日文等复杂的非 ASCII 字符

        
#### 29.4 Base64编码
#### Base64 是一种将二进制数据转换为可打印 ASCII 字符的编码方式，它将 3 个字节（24 位）的数据块转换为 4 个 Base64 字符
(每个字符 6 位),如果数据长度不是 3 的倍数，会使用 “=” 字符进行填充
* Base64 本身不是一种加密方式，它只是用于数据的编码转换，方便数据在不同系统或协议之间以文本形式传输或存储，而不会改变数据的内容实质
* 场景: 网络传输二进制数据(如图像、音频),若协议只支持文本数据传输时，Base64 编码可以将二进制数据转换为文本形式进行传输
* 数据存储的文本化：在存储数据时，当存储介质或数据库字段类型不支持二进制数据，而需要以文本形式存储时，可以使用 Base64 编码


#### 30. XCFramework 和Framework区别
#### XCFramework是苹果推出的新的Framework格式，与原有的Framework很相似，但是它可以为苹果旗下所有操作系统（iOS，macOS，iPadOS等），不同体系结构（arm64，armv7 ，i386）包含模拟器，甚至UIKit和AppKit App 提供统一的分发方式
* XCFramework特点: 

        跨平台支持: 支持多个平台的不同架构
        架构管理:允许开发者按照平台划分文件，每个平台只包含该平台所需的架构文件,简化了库文件的管理和分发过程，提高了编译和链接的效率
        易于使用: 只需要在Xcode项目中添加相应的.xcframework文件，然后按照需要进行配置即可

* XCFramework是一种容器格式，它包含了针对特定平台和架构编译的多个二进制文件，因此按平台和架构组合模式组织目录结构

![图片](https://github.com/WGFcode/WGFcodeNotes/blob/master/WGFcodeNotes/WGScreenshots/xcframework.png)

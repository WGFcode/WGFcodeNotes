...
* 操作队列OperationQueue并没有像GCD那样可以明确的创建串行队列和并发队列，通过初始化创建的操作队列，默认是并发的(默认的maxConcurrentOperationCount不等于1)，但是可以通过设置maxConcurrentOperationCount(最大的并发操作数)来达到串行的效果，如果该属性设置为1，则为串行效果
* maxConcurrentOperationCount设置操作队列的最大并发操作数，控制的不是并发线程的数量，而是一个队列中同时能并发执行的最大操作数；如果我们将其设置为1，其实操作队列效果就类似串行队列，每次只能执行一个任务并按照顺序执行，那么操作队列中的任务(操作)就是同步执行的；如果将其设置的值 > 1，那么操作队列内的任务就是异步执行的；如果将其设置的值 = 1，那么操作队列内的任务就是同步执行的； 不管 > 1 还是 < 1, 都不会阻塞操作队列外的线程；如果想阻塞队列外的线程，可以在添加操作到操作队列时，设置waitUntilFinished为true即可
* Operation实现多线程一般就是创建操作，将操作添加到操作队列即可实现异步多线程调用；如果想控制任务(操作)的执行顺序，那么可以通过添加操作之间的依赖来实现；如果操作里面嵌套了异步任务，那么为了保证该操作真正的执行晚上，需要借助信号量来实现

* Operation多线程比GCD多的功能点就是: 1.任务(操作)之间可以添加依赖 2.操作队列可以实现暂停/恢复/取消 队列中未执行的任务(操作)，注意取消的任务(操作)是不能恢复的 3.可以通过操作Operation取消单个任务(操作)，也可以通过操作队列取消多个任务(操作)

* Operation设置操作的优先级，并不能控制操作的执行顺序，只是去控制同一队列中进入就绪状态的操作(任务)的开始执行顺序，什么是就绪状态？就绪状态取决于操作之间的依赖关系。添加到队列中的操作基本都是处于准备就绪状态，而如果操作之间又添加了依赖关系，就要根据依赖关系来确定操作是否是就绪状态；因为优先级不能取代依赖关系。

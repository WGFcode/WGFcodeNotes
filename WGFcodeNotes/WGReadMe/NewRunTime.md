...
或+resolveClassMethod方法,并且返回值不为nil,即动态添加了方法,则将此标记为已动态解析过,然后继续走消息转发流程; 消息转发流程若找到直接调用,若没有找到,又会走到动态解析阶段,此时判断标记,发现已经动态解析过了,则直接进入方法转发阶段;若根本没有实现上面的两个方法中的一个或者返回值为nil,即动态方法解析阶段什么也没有处理,则会进入方法转发阶段

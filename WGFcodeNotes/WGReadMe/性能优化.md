...
* Data: 数据块(每一个segment的具体数据都保存在这里，这里包含了具体的代码、数据等等)
...
* LC_SEGMENT_64(__PAGEZERO): 空指针异常捕获段
        ...
        不能读、不能写、不能执行
* LC_SEGMENT_64(__LINKEDIT): 包含了动态链接库的原始数据，如符号、字符串、重定位表条目等
* LC_SEGMENT_64(__TEXT): 代码段；包含多个区section;常见的区如下

        __TEXT: 包含了可执行的代码和其他一些只读的数据（比如const 常量）
                可读、可执行、不可修改代码

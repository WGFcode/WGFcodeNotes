...
#### 如果按照ARC和MRC来区分的话，主要方式如下
    MRC: assign/retain/copy/readwrite/readonly/nonatomic/atomic
    ARC: assign/strong/weak/copy/readwrite/readonly/nonatomic/atomic
...
#### 总结：assign一般用来修饰基本数据类型，如果修饰对象，会出现野指针的问题；assign和weak的区别：就是weak在对象销毁的时，会自动将对象置为nil，而assign不会从而导致野指针问题（对象销毁了，指针指向了一个垃圾内存）;另外一个原因就是assgin修饰的基本数据类型，内存是分配在栈上的，栈内存的分配和销毁是由系统控制的，而对象类型的数据是分配在堆上的，用assign修饰的对象，在对象销毁时，对象的指针地址是还存在的，也就是说指针并没有被置为nil,
...
* @dynamic name1
        ...
        作用：告诉编译器不再生成成员变量_name1;只生成name1属性的getter/setter方法的声明;不再生成name1属性的getter/setter的实现

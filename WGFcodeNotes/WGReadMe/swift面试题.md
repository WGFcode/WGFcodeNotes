...
* 当混合使用值和引用类型时,结构体或枚举包含引用类型时它们需要的引用计数开销与他们包含引用类型的数量成正比
        ...
        let a = MyStruct()
        let anotherA = a
        NSLog("------\(CFGetRetainCount(a as CFTypeRef))")
        NSLog("------\(CFGetRetainCount(a.ref1))")
        NSLog("------\(CFGetRetainCount(a.ref2))")
        //打印结果：     
        ------1
        ------3      
        ------3
        ...
        let b = MyClass()
        let anotherB = b
        NSLog("------\(CFGetRetainCount(b as CFTypeRef))")
        NSLog("------\(CFGetRetainCount(b.ref1))")
        NSLog("------\(CFGetRetainCount(b.ref2))")
        ------3
        ------2
        ------2
* 上面的代码中发现 MyStruct结构体产生了两倍的引用计数成本
* 所以应当避免值类型包含引用类型的变量，因为它们违反了值的语义并产生额外的引用计数开销
...
#### 8. Any、AnyObject、AnyClass的区别
* Any: 表示任意类型，包括基本数据类型、结构体、枚举、类和函数等。适用于需要处理多种不同类型的值的情况；是一种非常通用的类型擦除机制
* AnyObject: 表示任意**类类型**的实例。适用于需要处理任意类实例的情况;AnyObject是Any的子集;只能存储类的实例
* AnyClass: 是一个用于表示任何类类型本身（不是类的实例）的元类型: AnyClass = AnyObject.Type;它不是类的实例，而是在类型层面上操作类
...
* any表示可以是任意类型(包括值类型/引用类型/函数类型等)，它引入了"抽象类型"的概念；它是一个非常通用的类型擦除器
* some用于表示返回某种特定类型，它引入了"具体类型"的概念；是一种存在类型
* Some是在 Swift 的不透明类型（Opaque Types）概念中出现的。它用于隐藏具体的类型信息，同时保证返回值的类型一致性。当一个函数返回Some类型时，
调用者知道返回值是某种特定类型，但具体是哪种类型被隐藏了
* some和any都用于类型擦除和泛型的实现
* 另外一种叫法: some代表了不透明类型;any代表了装箱类型
* some关键字引入了不透明类型，在编译时确定类型但对外隐藏了具体类型的实现细节;提升了性能，因为编译器在编译期就能确定实际的类型
* any关键字用于表示装箱类型，这种类型在运行时动态决定具体类型。这种灵活性带来了一些性能开销，因为需要在运行时进行类型判断
* any适用于在编译时无法确定具体类型或者需要在运行时处理多种类型的场景
* 如果可以在编译时确定类型且需要较好的性能，优先考虑使用some
* 在需要更高的灵活性或者必须在运行时确定类型时，any则是更好的选择
...
#### 28 swift中装箱、拆箱
#### 背景：值类型（如结构体和枚举）通常在栈上分配内存，它们的生命周期和作用域紧密相关。而引用类型（如类）在堆上分配内存，
并且通过引用计数来管理其生命周期

#### 频繁的装箱和拆箱操作可能会对性能产生一定的影响，因为涉及到内存分配（装箱时在堆上创建新对象）和类型检查（拆箱时）等操作。
在编写代码时，应该尽量避免不必要的装箱和拆箱，以提高代码的性能和效率

* 装箱: 将值类型转换为引用类型

        let age = 18
        //这里就发生了装箱 装箱后的对象在堆上分配内存，并且会有相应的引用计数管理机制
        var newAge: Any = age
##### 当发生装箱时，Swift 会在堆上创建一个新的对象来包装原来的值类型。这个新对象通常是一个类的实例，它包含了原来值类型的值
例如，对于上面的Int装箱到Any的情况，在底层会创建一个类似Box<Int>（这只是一个示意，实际实现可能更复杂）的类实例，其中Box类有一个成员变量来存储Int的值。
这个Box类实例的引用计数会被初始化为 1，并且通过自动引用计数（ARC）机制来管理其生命周期

* 拆箱: 将引用类型转换为值类型

        当你从一个已经装箱的对象中获取原来的值类型的值时，就需要进行拆箱
        //这里发生了拆箱
        if let intAge = newAge as? Int {
            NSLog("intAge:\(intAge)")
        }
        通过as?操作符尝试将anyValue拆箱回Int类型。如果anyValue确实是之前装箱的Int类型，那么就可以成功拆箱并获取到原来的值
##### 拆箱的底层实现涉及到类型检查和值的提取。在上面的例子中，as?操作符会首先检查anyValue的实际类型是否是Int。如果是，它会从装箱的对象
（如前面提到的Box<Int>类实例）中提取出Int的值并返回；拆箱操作需要注意类型安全。如果在拆箱时类型不匹配，可能会导致运行时错误
例如，如果你试图将一个装箱的String类型的Any对象拆箱为Int，会得到nil（如果使用as?操作符）或者触发运行时错误（如果使用as操作符）。

#### 哪些会触发装箱拆箱操作
* Any类型、AnyObject类型会触发装箱
* 闭包捕获值

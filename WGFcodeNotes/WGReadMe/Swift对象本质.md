...
#### 4.6 属性包装器 @propertyWrapper
#### 属性包装器是将定义属性和对属性的限定行为分离开来，属性包装器接管属性的get、set方法，通过get、set方法来对属性进行做限定行为处理
实际就是将属性的设置和获取逻辑封装在一个单独的类型中，然后将这个类型应用到属性上；这种方式使得代码更加简洁、可读、易于维护
* 使用属性包装器有两个条件必须满足

        属性包装器必须用@propertyWrapper关键词定义
        属性包装器内部必须具有wrappedValue属性，且至少有一个get方法
* 属性包装器最好是用struct结构体；Struct是值类型，会自动管理饮用，不用担心循环引用
* 协议/extension扩展/enum/不支持使用属性包装器

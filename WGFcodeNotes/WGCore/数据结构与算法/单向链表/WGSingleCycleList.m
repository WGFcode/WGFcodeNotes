...
/*
    ...
    first  ---> next --->next ---->next ---->next ----->next //----->null
                 ...
                 -----------------------------------------
 */
#import "WGSingleCycleList.h"
#import "WGNode.h"
/// 找不到元素
static int ELEMENT_NOT_FOUND = -1;

@interface WGSingleCycleList()
{
    int size;        //结点个数
...
@implementation WGSingleCycleList
//MARK: 获取链表元素的个数
-(int)size {
    return size;
}
...
-(int)remove:(int)index {
    ...
    if (index == 0) {
        ...
        if (size == 1) { //就一个元素需要删除
            ...
            //拿到最后一个结点的元素,让它的next指向删除结点的下一个结点; 下面顺序不能搞反
            WGNode *lastNode = [self getNodeWithIndex:size - 1];
